...
metadata:
  cogitum:
    ...
    related_skills: [systematic-debugging, node-inspect-debugger, systematic-debugging]
---
...
- A long-running process (cogitum gateway, tui_gateway) misbehaves and you can't restart it
...
The cogitum test runner and pytest both support this:
...
For long-lived processes: Cogitum gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source /home/bb/cogitum/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Cogitum you have two practical options:
...
```json
{
  "name": "Attach to Cogitum",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/cogitum" }
  ]
}
```
...
Easiest: add `breakpoint()` near the suspect line, then run `cog` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `cog --tui`)
...
debugpy.wait_for_client()
```
Start `cog --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Cogitum subagents, debug one process at a time.
