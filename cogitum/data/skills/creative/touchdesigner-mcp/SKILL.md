...
metadata:
  cogitum:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, manim-video]
...
```
Cogitum -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${HERMES_HOME:-$HOME/.cogitum}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Cogitum config (if missing)
...
3. **Restart Cogitum session** to pick up the new MCP server
