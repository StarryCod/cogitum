...
description: "Operate the Teams meeting summary pipeline via Cogitum CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions."
version: 1.1.0
...
```bash
cog teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
cog teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
cog teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
cog teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

cog teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
cog teams-pipeline delete-subscription <sub-id>
cog teams-pipeline maintain-subscriptions            # renew near-expiry ones
cog teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `cog teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `cog cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
