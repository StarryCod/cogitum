...
2. Set `LINEAR_API_KEY` in your environment (via `cog setup` or your env config)
...
```bash
SCRIPT=$(dirname "$(find ~/.cogitum -path '*skills/productivity/linear/scripts/linear_api.py' 2>/dev/null | head -1)")/linear_api.py
